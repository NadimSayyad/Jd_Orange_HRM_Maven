...
public class ScreenShotExample {
	...
	String Relativepath_failure = filePath_failure + "\\screenshotFailure";
	String filePath_Success = filePath_failure + "\\screenshotSuccess";
	...
	public void CaptureScreenShot(ITestResult result) throws IOException {
		if (ITestResult.SUCCESS == result.getStatus()) {
			/*
			 * Path path = Paths.get(Relativepath_failure); Files.delete(path);
			 */
			FileUtils.cleanDirectory(new File(Relativepath_failure)); 
			...
			/*
			 * Path path = Paths.get(filePath_Success); Files.delete(path);
			 */
			FileUtils.cleanDirectory(new File(filePath_Success)); 
			File Browserscreenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
